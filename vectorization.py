...
def objective_for_snap(alpha, V, design_part):
    ...
    u = np.linspace(0.0, 1.0, 100)
    curve = np.transpose(curve.evaluate_multi(u))
    res = max(directed_hausdorff(design_part, curve)[0], directed_hausdorff(curve, design_part)[0])
    return res

def close_curve_for_snap(V, design_part):
    t = [unit(V[0] - V[1]), unit(V[-1] - V[-2])]
    bound = 1
